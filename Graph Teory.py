...
def DFS(graph, start, end,path, shortest, toPrint = False):
    """Assume graph is a Diagraph; start and end are nodes;
    ...
    for node in graph.childrenOf(start):
        if node not in path:
            ...
            print('Alredy visited', node)
    return shortest

def shortestPath(graph, start, end, toPrint=False):
    """Assume graph is a Diagraph; start and end are noodes
       Returns a shortest path from start to dne in graph"""
    return DFS(graph,start, end, [], None, toPrint)

def run_testSP(source, destination):
    ...
    if sp != None:
        ...
        print('There in no path from', source, 'to', destination)

run_testSP("Chicago", "Boston")
