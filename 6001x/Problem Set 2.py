...
remaining_balance = calculate_remaining_balance(balance, annualInterestRate, monthlyPaymentRate)
...
def find_minimum_fixed_payment(balance, annualInterestRate):
    monthly_interest_rate = annualInterestRate / 12.0
    ...
    return minimum_fixed_payment


lowest_payment = find_minimum_fixed_payment(balance, annualInterestRate)
print("Lowest Payment: ", lowest_payment)

########################################################################################################################

#You('ll notice that in Problem 2, your monthly payment had to be a multiple of $10.
#Why did we make it that way? You can try running your code locally so that the payment can be
#any dollar and cent amount (in other words, the monthly payment is a multiple of $0.01).
...
#What we ultimately pay must be greater than what we would
#('ve paid in monthly installments, ''because the interest was compounded on the balance we didn')t pay off each month.
...
def calculate_remaining_balance(balance, annualInterestRate, monthlyPayment):
    monthly_interest_rate = annualInterestRate / 12.0
    for month in range(12):
...
def find_minimum_fixed_payment(balance, annualInterestRate):
    ...
    while upper_bound - lower_bound > epsilon:
        ...
        remaining_balance = calculate_remaining_balance(balance, annualInterestRate, monthlyPayment)
